...
void logError(Error a, char * format, ...);
void logError(Error a, char * format, ...) {
	va_list argptr;
...
ByteArray * bytes = createNewByteArrayUsingDataCopy(data, 188, logError);
Alert * alert = newAlertFromData(bytes, logError);
if(deserializeAlert(alert) != 188){
...
if (memcmp(getByteArrayData(alert->displayedComment),"See bitcoin.org/feb20 if you have trouble connecting after 20 February",70)) {
printf("DESERIALISATION DISPLAYED COMMENT FAIL\n0x");
char * d = (char *)getByteArrayData(alert->displayedComment);
int x;
for ( x = 0; x < 70; x++) {
printf("%c",d[x]);
}
