...
SeedPeers* createNewSeedPeers(void (*onErrorReceived)(Error error,char *,...)) {
	SeedPeers * self = malloc(sizeof(*self));
		if (!self) {
			onErrorReceived(ERROR_OUT_OF_MEMORY,"Cannot allocate %i bytes of memory\n",sizeof(*self));
			return NULL;
		}
		getObject(self)->destroy = destroySeedPeers;
		if (initializeSeedPeers(self,onErrorReceived))
			return self;
		free(self);
		return NULL;
     /*  this.params = params;*/
...
{
	self->pnseedIndex=0;
	self->seedAddrs=malloc(NUMBER_OF_SEED_PEERS*4);
	if (!self->seedAddrs) {
			onErrorReceived(ERROR_OUT_OF_MEMORY,"Cannot allocate %i bytes of memory\n",NUMBER_OF_SEED_PEERS*4);
			return FALSE;
		}
	uint32_t addresses[NUMBER_OF_SEED_PEERS] =
    {
	...
	memcpy( self->seedAddrs,addresses,NUMBER_OF_SEED_PEERS*4);
...
NetworkAddress* convertAddress(uint32_t seed,void (*onErrorReceived)(Error error,char *,...))  {
		...
		uint8_t  v4addr[16]={0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
        v4addr[12] = (uint8_t) (0xFF & (seed));
        v4addr[13] = (uint8_t) (0xFF & (seed >> 8));
        v4addr[14] = (uint8_t) (0xFF & (seed >> 16));
        v4addr[15] = (uint8_t) (0xFF & (seed >> 24));

        uint32_t time=0;
        ByteArray * ip=createNewByteArrayUsingDataCopy(v4addr,16,onErrorReceived);
        uint16_t port=8333;
        uint64_t services=0;
        NetworkAddress* netAddr=createNewNetworkAddress(time,ip,port,services,onErrorReceived);
...
NetworkAddress* nextPeerAddress(SeedPeers * self,void (*onErrorReceived)(Error error,char *,...))
{
		if(!self)
			return NULL;
        if (self->pnseedIndex >= NUMBER_OF_SEED_PEERS) return NULL;
        return convertAddress(self->seedAddrs[self->pnseedIndex++],onErrorReceived);
}
...
NetworkAddress** getAllPeers(SeedPeers * self,void (*onErrorReceived)(Error error,char *,...))
{
		if(!self)
			return NULL;
		NetworkAddress** addresses = malloc(NUMBER_OF_SEED_PEERS*sizeof(*addresses));
        int i;
		for (i = 0; i < NUMBER_OF_SEED_PEERS; ++i) {
            addresses[i] = convertAddress(self->seedAddrs[i],onErrorReceived);
        }
        return addresses;
 }
